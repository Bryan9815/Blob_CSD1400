...
void BossInit(Boss *currentboss, int health, float size) //function to set variables of boss(es)
{
	...
	currentboss->Hitbox.radius = currentboss->Size = size;
...
void BossMovement(Boss* currentboss, Player player) //boss slowly moves toward player
{
	if ((int)CP_System_GetMillis() % 1000 >= 700) //boss moves for 0.3s every 1s (to be balanced)
	{
		...
		if (COL_Dist(currentboss->Hitbox, player.hitBox) >= (currentboss->Size + player.radius + distance)) //boss destination does not overlap with player
		{
		...
		{
			distance = COL_Dist(currentboss->Hitbox, player.hitBox) - (currentboss->Size + player.radius); //find distance from player
