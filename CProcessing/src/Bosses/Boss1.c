...
void AttackNear(Boss* armorboss, Player* player) //attacks a radius around boss
{
	...
	else //attack finished
	{
		...
		armorboss->Hitbox.radius = armorboss->Radius; //reset hitbox
...
void B1_StateChange(Player player, Boss* currentboss) //this determines WHEN the boss does its actions
{
	...
	else if (StateTimer >= (6 * CP_System_GetFrameRate()) && PlayerDist <= (BossRange * 2)) //if player is within distance after 6 sec
	{
		currentboss->State = ATTACK_NEAR;
		StateTimer = 0;
	}
	else if (StateTimer >= (6 * CP_System_GetFrameRate()) && PlayerDist > (BossRange * 2)) //if player is beyond distance after 6 sec
	{
