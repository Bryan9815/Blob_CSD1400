...
void CameraInit(CP_Vector *targetPos, CameraMode cameraMode)
{
	...
	currentPosition = CP_Vector_Set(roundf(-(targetPos->x - centerX)), roundf(-(targetPos->y - centerY)));
...
void CameraUpdate(CP_Vector* targetPos, Fader* fader)
{
	...
	{
		currentPosition = CP_Vector_Set(roundf(-(targetPos->x - centerX + (CP_Random_RangeFloat(-shakeValue, shakeValue)))), roundf(-(targetPos->y - centerY + (CP_Random_RangeFloat(-shakeValue, shakeValue)))));
		cameraOffset = CP_Vector_Subtract(currentPosition, cameraPos);
		cameraOffsetDist = CP_Vector_Distance(currentPosition, cameraPos);
